...
Conforme ilustrado na \autoref{fig:network_dantzig}, o Problema de Transporte pode ser interpretado como um problema de fluxo em rede.
...
No estudo eles aplicam o que eles chamam de roteirização, descrito como atividades que procuram melhorar trajetos para um veículo realizar dentro de uma malha, com o intuito de reduzir tempo e/ou distância. No estudo, ele se propõem a otimizar o percurso focando na minimização de custo financeiro das rotas, por meio da minimização da distância percorrida. Para realização dos cálculos utilizaram como referencia a tabela do frete disponibilizada pela ANTT (Agência Nacional de Transportes Terrestres).
...
O  modelo de Lima et al. (2019) é limitado geograficamente a um subconjunto de municípios do Estado de Minas Gerais, sem considerar aplicação em nível nacional com diferentes modais de transportes e etapas intermunicipais, estaduais e intramunicipais de distribuição.
...
Conforme ilustrado na \autoref{fig:grafico_bipartido_shukla}, o modelo introduz nós artificiais de origem (\textit{source}) e destino (\textit{target}) que balanceiam a oferta e a demanda agregada em dias de serviço ($ltc\_days$). A soma das capacidades dos depósitos (\textit{depot}) representa a oferta agregada e a soma das demandas das clínicas (\textit{LTCF}) representa a demanda agregada. Para respeitar a capacidade máxima de trabalho, cada depósito é desdobrado em nós auxiliares que representam as suas equipes em campo (frentes SNF e ALF). Por fim, as arestas que ligam essas equipes às clínicas carregam o peso logístico que o modelo busca minimizar, garantindo que as restrições de tempo e capacidade não sejam violadas.

O peso das arestas é definido pelo produto entre os dias de serviço ($ltc\_days$) e a distância ($dist$) entre o depósito e a clínica:

\[
\text{Minimizar } Z = \sum (dist(a,m) \times ltc\_days(m))
\]

Na segunda fase, o agendamento das visitas é resolvido por um algoritmo heurístico que estima o tempo total de operação diária ($time$) para cada equipe. A fórmula considera a velocidade média de deslocamento (estimada em 20 milhas por hora), a velocidade de aplicação (8 doses por hora) e um tempo adicional de preparação da vacina antes de ela ir para o braço do paciente e que depende do tipo de vacina:
...
\begin{itemize}
    ...
    \item $C_i$ é a influência do grupo $i$ na propagação do vírus;
...
Dessa forma, essa restrição garante que se um veículo visita 4 localidades ($|V_s| = 4$), ele só pode percorrer no máximo 3 ($4-1=3$) viagens entre essas localidades, forçando o retorno ao depósito para completar a rota.

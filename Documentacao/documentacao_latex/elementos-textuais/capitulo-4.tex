...
\[
U^{\text{rod}}, \quad U^{\text{aereo}} \in \mathbb{R}_{>0}
\]
...
\[
q_{jp} = W_{j} * U^{rod} \in \mathbb{R}_{\geq 0}
\]
...
As variáveis de decisão para a etapa intramunicipal ($z_{jpv}$) e para a demanda não atendida ($f_{pv}$) são reaproveitdas neste modelo.
...
\[
u_{ik}^{\text{rod}}, \quad u_{ik}^{\text{aereo}} \in \mathbb{R}_{\geq 0}
\]
...
u_{ik} =
\begin{cases}
u_{ik}^{\text{rod}}, & \text{se } C_{ik}^{\text{rod}} \le C_{ik}^{\text{aereo}} \\
u_{ik}^{\text{aereo}}, & \text{caso contrário}
\end{cases}
\end{equation}
...
onde a primeira, segunda e terceira parcelas representam, respectivamente, os custos da primeira, segunda e tereira etapa do fluxo, e a quarta parcela penaliza o não atendimento da demanda.

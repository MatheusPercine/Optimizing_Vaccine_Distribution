...
Além das diferenças de tempo entre os modais de transporte, é necessário considerar que a \textbf{capacidade de transporte} também pode variar de acordo o número de veículos disponíveis para cada modal.
...
\[
W_{i}^{\text{rod}}, \quad W_{i}^{\text{areo}} \in \mathbb{Z}_{>0}
\]
...
onde:
\begin{itemize}
    \item $U^{\text{rod}}$ representa a capacidade média de carga de um veículo domodal rodoviário;
    \item $U^{\text{areo}}$ representa a capacidade média de carga de um veículo do modal aéreo.
\end{itemize}

Neste trabalho, a capacidade não representa apenas a capacidade física de um único veículo individual, mas sim uma \textbf{capacidade operacional agregada} disponível ao longo do horizonte de planejamento considerado pelo modelo.

Dessa forma, a capacidade efetiva de transporte por modal é definida como:

\[
u_{ij}^{\text{rod}} = W_i^{\text{rod}} \cdot U^{\text{rod}}
\]

\[
u_{ij}^{\text{areo}} = W_i^{\text{areo}} \cdot U^{\text{areo}}
\]
...
Para representar esse comportamento de forma mais realista, introduz-se uma variável de folga associada à demanda não atendida. Seja:
...
onde:
\begin{itemize}
    \item $f_{jv}$ representa a quantidade faltante da vacina do tipo $v$ no município de destino $j$.
\end{itemize}

A variável $f_{jv}$ permite flexibilizar a restrição de demanda, evitando que o modelo se torne inviável quando não houver capacidade suficiente para atender integralmente todos os municípios.

Entretanto, permitir faltas sem qualquer penalização faria com que o modelo pudesse minimizar o tempo de transporte simplesmente não enviando nenhuma vacina. Para evitar esse comportamento, introduz-se uma constante de penalização $M$, cujo valor deve ser suficientemente elevado em relação aos custos de transporte.
...
pois, nesse caso, todas as vacinas ofertadas seriam obrigatoriamente transportadas. Em um cenário real, contudo, o lote excedente pode não ser integralmente demandado, gerando sobras locais legítimas.

\textbf{A segunda restrição} é a \textbf{restrição de demanda} da vacina $v$ por cada município $j$. 
O número de vacinas recebidas, somado à variável de falta, deve ser igual à demanda total do município. Assim, temos:

\begin{equation*}
    \sum_{i} x_{ijv} + f_{jv} = d_{jv}, \quad \forall j \in J,\; v \in V
\end{equation*}

onde:
\begin{itemize}
    \item $d_{jv}$ representa a quantidade de vacinas do tipo $v$ demandadas no município $j$;
    \item $f_{jv}$ representa a quantidade não atendida da demanda;
    \item $J$ representa os municípios $j$ mapeados.
\end{itemize}
...
\begin{equation}
    \min z = 
    \sum_{v} \sum_{i} \sum_{k} C_{ik} \cdot x_{ikv} \quad + \quad 
    \sum_{v} \sum_{k} \sum_{j} C_{kj} \cdot y_{kjv} \quad + \quad 
    \sum_{v} \sum_{j} M \cdot f_{jv}
    \label{eq:intermunicipal_estendido_sem_penalidade}
\end{equation}

onde a primeira parcela representa o custo da primeira etapa do fluxo, a segunda parcela representa o custo da segunda etapa e a terceira parcela penaliza o não atendimento da demanda.

ou, com a penalidade:

\begin{equation}
    \min z = 
    \sum_{v} \sum_{i} \sum_{k} C_{ik} \cdot x_{ikv} \cdot \rho_{iv} \quad + \quad \sum_{v} \sum_{k} \sum_{j} C_{kj} \cdot y_{kjv} \quad + \quad
    \sum_{v} \sum_{j} M \cdot f_{jv}
    \label{eq:intermunicipal_estendido_com_penalidade}
\end{equation}

A penalidade de expiração é aplicada na origem, pois o risco de expiração está associado ao tempo restante no município de origem.
...
\begin{equation}
\sum_{k} y_{kjv} + f_{jv} = d_{jv}, \quad \forall j \in J,\; v \in V
\end{equation}
...
\begin{itemize}
    ...
    \item $W_{j}$ representa o número de veículos disponíveis para transporte no município $j$.
\end{itemize}
...
De forma análoga aos modelos intermunicipais, introduz-se a variável de falta $f_{pv}$, que representa a quantidade não atendida da vacina do tipo $v$ no posto $p$.
...
Diferentemente dos modelos intermunicipais anteriores, neste caso a função objetivo possui um somatório triplo associado à penalização de falta, pois a variável $f_{pv}$ está definida no nível dos postos de vacinação pertencentes a cada município $j$.

O objetivo é minimizar o tempo total de distribuição das vacinas dentro do município e, ao mesmo tempo, minimizar a demanda não atendida por meio da penalização $M$.
...
\begin{equation}
\min z = \sum_{v} \sum_{j} \sum_{p \in P_j} C_{jp} \cdot z_{jpv} \cdot \rho_{jv} + \sum_{v} \sum_{j} \sum_{p \in P_j} M \cdot f_{pv}
...
A \textbf{restrição de demanda} estabelece que sua demanda total é igual a o número de vacinas enviadas mais um valor referente à demanda não suprida:
...
Diferentemente dos modelos intermunicipais, não há necessidade de somatório sobre os fluxos recebidos, pois cada posto $p$ recebe vacinas exclusivamente do município $j$ ao qual pertence.
...
+
\sum_{v} \sum_{j} \sum_{p \in P_j} M \cdot f_{pv}
\label{unificado_estendido_sem_penalidade}
\end{equation}
...
+
\sum_{v} \sum_{j} \sum_{p \in P_j} M \cdot f_{pv}
\label{unificado_estendido_com_penalidade}
\end{equation}

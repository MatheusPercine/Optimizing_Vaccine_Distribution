...
Além das diferenças de tempo entre os modais de transporte, é necessário considerar que a \textbf{capacidade de transporte} também pode variar de acordo com o número de veículos disponíveis para cada modal em um município.
...
\subsubsection{fatores de penalização associados ao tempo restante de validade da vacina}
...
\item $\rho_{jv}$ é a penalidade da vacina do tipo $v$ no município $j$.
\end{itemize}
...
Dessa forma, $\rho_{iv}$ se refere à penalização associada à etapa intermunicipal e $\rho_{jv}$ se refere à penalização associada à etapa intramunicipal, já que o tempo de expiração variou entre uma etapa e outra.
...
O número de vacinas enviadas pode ser menor que o número de vacinas ofertadas no local de origem e, no máximo, pode-se transportar uma quantidade igual à quantidade ofertada pela origem. Em um cenário real, contudo, o lote excedente pode não ser integralmente demandado, gerando sobras locais legítimas.
...
A \textbf{restrição de capacidade agregada} de entrega de vacinas da origem $i$ ao destino $j$, segue a lógica de que o número de vacinas transportadas não pode exceder a capacidade máxima agregada. Assim, temos:
...
\begin{equation*}
z_{jpv}, f_{pv} \in \mathbb{Z}_{\geq 0}, \quad \forall j \in J, p \in P_j, v \in V
\end{equation*}
...
Esta restrição descreve o que acontece com o estoque de vacinas de cada município que pode receber vacinas de outras localidades ou fornecê-las para outros municípios ou postos. Logo, a lógica matemática estabelece que a soma de tudo o que sai do município (seja para o consumo interno em seus postos ou para exportação a outras localidades) não pode ser superior à soma de tudo o que o município dispõe (seu estoque inicial somado ao que ele recebeu de fora durante o processo). 
...
\begin{equation*}
x_{ijv}, z_{jpv}, f_{pv}\in \mathbb{Z}_{\geq 0}
\end{equation*}
...
A separação dessas variáveis é necessária, pois os fluxos de entrada e saída de um centro intermediário podem apresentar distribuições diferentes mesmo que esteja sendo considerada a conservação do fluxo entre etapas. Por exemplo, para um determinado tipo de vacina $v$ em um centro $k$, pode-se ter:
...
Logo, o tempo de expiração da vacina que chega ao município $j$ é determinado unicamente pela validade média remanescente no seu respectivo centro estadual emissor subtraída do tempo de transporte logístico entre as duas localidades. Logo, temos:
...
onde a primeira, segunda e terceira parcelas representam, respectivamente, os custos da primeira, segunda e terceira etapas do fluxo, e a quarta parcela penaliza o não atendimento da demanda.
...
A \textbf{Restrição de Conservação e Disponibilidade de Estoque} também deve ser implementada neste modelo seguindo um raciocínio análogo do modelo anterior e fazendo-se as adaptações necessárias considerando o intermédio dos centros estaduais. Dessa forma, a restrição é expressa por:
...
A \textbf{primeira restrição de capacidade agregada} é relativa à capacidade de transporte da primeira etapa do fluxo e, nesta restrição, a quantidade de vacinas transportadas não pode ultrapassar esse limite. Logo, temos:
...
A \textbf{terceira restrição de capacidade agregada da etapa intramunicipal} também é reaproveitada do modelo anterior:
...
Não é necessária uma restrição de viabilidade temporal relativa a variável $y_{kjv}$. Caso o tempo de transporte entre o município de origem e o centro estadual exceda o tempo restante até a expiração da vacina, o envio é proibido, pois a restrição de conservação de fluxo garante que, na ausência de entrada viável, não haverá saída correspondente, assegurando a consistência do modelo.
...
\begin{equation*}
x_{ikv}, y_{kjv}, z_{jpv}, f_{pv} \in \mathbb{Z}_{\geq 0}, \quad \forall i \in I, k \in K, j \in J, p \in P_j, v \in V
\end{equation*}

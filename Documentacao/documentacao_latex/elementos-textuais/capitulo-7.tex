...
O Modelo 5 (sem penalidade) foi idealizado sob uma ótica mais realista que os demais modelos e, por isso, é válido analisar a complexidade computacional gerada por essa abordagem.

O Modelo 5 exige a operação com três variáveis de decisão tridimensionais ($x_{ikv}$, $y_{kjv}$ e $z_{jpv}$) e uma variável bidimensional de falta ($f_{pv}$). Como consequência, a quantidade de vezes que as variáveis aparecem na função objetivo e o tamanho da matriz de restrições crescem de forma multiplicativa em relação ao tamanho dos conjuntos de municípios de origem ($|I|$), municípios de destino ($|J|$), centros estaduais ($|K|$), postos de vacinação ($|P(j)|$) e tipos de vacina ($|V|$).
...
\[
= 5103 + 5103 + 378 + 378 = 10.962 \text{ vezes}
\]

\textbf{Quantidade de linhas na matriz de restrições:}
\begin{align*}
& (27 \cdot 7) \quad && \text{(Oferta Intermunicipal)} \\
+ \;& (27 \cdot 7) \quad && \text{(Conservação do Fluxo no Estado)} \\
+ \;& (54 \cdot 7) \quad && \text{(Demanda nos Postos de Saúde)} \\
+ \;& (27 \cdot 27) + (27 \cdot 27) + (54) \quad && \text{(Capacidades Físicas de Transporte)} \\
+ \;& (27 \cdot 27 \cdot 7) \quad && \text{(Consistência Estadual)} \\
+ \;& (27 \cdot 7) \quad && \text{(Balanço de Fluxo Municipal)} \\
+ \;& (27 \cdot 27 \cdot 7) + (54 \cdot 7) \quad && \text{(Viabilidade Temporal)}
\end{align*}
...
O tamanho da instância é a soma total das variáveis de decisão inteiras geradas pelos dados de entrada, ou seja, 10.962.

Logo, no pior caso, o tempo de execução pode ser:

\[
\text{tempo} \sim O(2^{10962}) \quad \text{(ou pior)}
\]

\textbf{Tamanho total da matriz do problema (Variáveis $\times$ Restrições):}
\[
10.962 \times 13.041 = 142.954.842 \text{ células}
\]
...
\begin{itemize}
    ...
    \item \textbf{Incapacidade de Previsão do Trânsito:} O modelo considera a velocidade média dos modais ($\theta_{rod}$ e $\theta_{aereo}$) como parâmetros constantes. Porém, na realidade urbana, o trânsito é um sistema dinâmico e imprevisível, pois está suscetível a intempéries climáticas, acidentes, bloqueios em rodovias, obras e grandes eventos (como partidas de futebol ou festividades), que podem travar o trânsito nas redondezas de rodovias e aeroportos, impactando drasticamente o tempo de deslocamento $\tau_{ab}$.

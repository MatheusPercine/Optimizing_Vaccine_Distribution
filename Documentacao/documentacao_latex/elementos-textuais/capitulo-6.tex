...
Os dados dos resultados foram exportados no formato csv, na pasta \texttt{``resultados\_caso\-\_regional''}, e para suas exibições foi utilizada a ferramenta Power BI que é capaz de gerar diversos gráficos com certa praticidade e tendo arquivos do tipo csv como entrada. Outros dados também foram impressos pelo terminal do python. 
...
Os tempos máximos são aproximadosss pelo valor do Limite Superior. 

Conforme ilustrado na \autoref{fig:grafico1_powerbi}, podemos verificar que os modelos com penalidades possuem tempos maiores do que os modelos sem penalidades. Isso se deve ao fato de que a penalidade na função objetivo tem o efeito de priorizar o atendimento dos postos de saúde, mesmo que isso implique em rotas mais longas ou mais demoradas.

Observa-se também que as execuções do modelo 2 (com intermédio de estados) possuem um tempo maior do que as execuções do modelo 1 (sem intermédio de estados), o que era esperado, já que o modelo 2 incorpora mais etapas logísticas do que o modelo 1. Porém, agora conseguimos medir em quanto esse impacto é significativo.

Entre os modelos 1 e 2, ambos sem penalidade, a diferença entre os tempos máximos registrados é de $3{,}008$ horas ($11{,}003 - 7{,}995$). Tomando o tempo do Modelo 1 como referência do total ($100\%$), o acréscimo percentual é calculado explicitamente por:

$$ \frac{11{,}003 - 7{,}995}{7{,}995} = \frac{3{,}008}{7{,}995} \approx 0{,}376 $$

Isso significa que o modelo 2 tem um tempo aproximadamente $37{,}6\%$ maior em relação ao modelo 1.

Considerando ambos os modelos com penalidade, a diferença entre os tempos é de $2{,}683$ horas ($12{,}530 - 9{,}847$). Utilizando novamente o modelo 1 como base de cálculo ($100\%$), a proporção de aumento é dada por:

$$ \frac{12{,}530 - 9{,}847}{9{,}847} = \frac{2{,}683}{9{,}847} \approx 0{,}272 $$

Isso significa que, sob o efeito da penalidade, o modelo 2 tem um tempo aproximadamente $27{,}2\%$ maior.
...
Este gráfico da \autoref{fig:grafico2_powerbi} denota uma grande diferença no valor da função objetivo entre um modelo sem penalidade e sua versão com penalidade, pois o modelo com penalidade é severamente punido, por diversas vezes, a cada hora que uma vacina com validade curta passa em trânsito.
...
Como ilustrado na \autoref{fig:grafico4_powerbi}, o total de doses aumentou conforme o modelo apresentava mais etapas, isso significa que o modelo 2 (com intermédio de estados) movimenta mais doses que o modelo 1 (sem intermédio de estados) porque uma mesma vacina é contabilizada duas vezes na viagem intermunicipal (uma vez indo para o estado, outra vez saindo do estado). 

As barras True e False são exatamente da mesma altura dentro de um mesmo modelo. Isso mostra que, a penalidade pode influenciar a escolha de rotas, mas não desiste de entregar as vacinas.
...
Como evidenciado na \autoref{fig:grafico5_powerbi}, os modelos, há predominância do modal rodoviário, por conta da predominância de rotas de curta distância intramunicipais (rotas da forma $j \to p$).
...
\subsection{Resultados do Modelo 1 (Sem Intermédio de Estados)}

\begin{figure}[H]
    \centering
    \caption{Os 10 Maiores Tempos Por Rota - Modelo 1 (Sem Intermédio de Estados)}
    \includegraphics[width=16.0cm,height=12.5cm]{figuras/9-maiores_tempos_mod1.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:grafico10_powerbi}
\end{figure}

Conforme ilustrado na \autoref{fig:grafico10_powerbi}, no cenário sem penalidade, os maiores tempos de rota concentram-se em eixos que interligam capitais do Centro-Oeste e Nordeste em direção a Manaus (ex: Campo Grande $\to$ Manaus, Goiânia $\to$ Manaus, São Luís $\to$ Manaus), com tempos estabilizados na faixa de 5 a 6 horas.

No cenário com penalidade, o Gurobi altera completamente o conjunto. Surgem eixos de longuíssima distância partindo do Sudeste, como por exemplo São Paulo $\to$ Boa Vista, São Paulo $\to$ Manaus e São Paulo $\to$ Belém. Na presença da penalidade, o teto do eixo vertical de custos em horas sobe de 6 horas para 8 horas. Isso significa que o solver aceita ativar rotas mais distantes para priorizar a entrega de lotes de vacinas com alto risco de vencimento.
...
\begin{figure}[H]
    \centering
    \caption{Os 10 Menores Tempos Por Rota - Modelo 1 (Sem Intermédio de Estados)}
    \includegraphics[width=16.0cm,height=12.5cm]{figuras/11-menores_tempos_mod1.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:grafico14_powerbi}
\end{figure}

Na \autoref{fig:grafico14_powerbi}, diferentemente do que ocorre no gráfico de maiores tempos, a análise das rotas mais rápidas revela que a introdução da penalidade de perecibilidade não altera a configuração logística da base da rede. Em ambos os cenários (com e sem penalidade), o conjunto das 10 rotas mais curtas é rigorosamente idêntico.

Observa-se que todas as rotas listadas são exclusivamente intramunicipais (fluxos do tipo $j \to p$), conectando as capitais aos seus respectivos postos de vacinação locais. O eixo de custo em horas apresenta um comportamento estático, formando uma barreira alinhada em, aproximadamente, 2 horas para todas as colunas. Esse fenômeno é explicado pela formulação matemática: como a distância geográfica dentro do mesmo município é muito curta, o custo total da rota passa a ser dominado pelo tempo fixo de processamento logístico do modal rodoviário ($\delta^{rod} = 2,0$ horas).

Os picos de volumes de doses em grandes centros populacionais (como São Paulo e Rio de Janeiro) indicam que estes locais recebem volumes massivos de doses, pois suas demandas são maiores.

\begin{figure}[H]
    \centering
    \caption{Percentual de Tipos de Rota - Modelo 1 (Sem Intermédio de Estados)}
    \includegraphics[width=16cm,height=5.5cm]{figuras/7-percentual_rotas_mod1.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:grafico11_powerbi}
\end{figure}

Na \autoref{fig:grafico11_powerbi}, verifica-se predomínio, em, aproximadamente 75\%, das rotas do tipo $jp$ ($j \to p$), no qual o município realiza o abastecimento de seus postos. O restante (aproximadamente 25\%) é preenchido pelas rotas do tipo $ij$ ($i \to j$), o que corresponde a casos em que um município precisou pegar doses da oferta de outro município.

Vacina do maior tempo de entrega e rotas gargalo por etapa:

\begin{figure}[H]
    \centering
    \caption{Informações do Terminal Com Python - Modelo 1 (Sem Intermédio de Estados) sem penalidade}
...
\begin{figure}[H]
    \centering
    \caption{Informações do Terminal Com Python - Modelo 1 (Sem Intermédio de Estados) com penalidade}
    \includegraphics[width=13.5cm,height=5.0cm]{figuras/modelo_1_true.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:terminal8_python}
\end{figure}

Considerando a \autoref{fig:terminal7_python} e a \autoref{fig:terminal8_python}, verifica-se que rotas gargalo e seus detalhamentos são completamente diferentes entre uma versão de modelo sem penalidade e sua versão com penalidade. O tipo de vacina da entrega mais demorada é a mesmo entre as duas saídas. Todas as demandas foram atendidas.
...
\begin{figure}[H]
    \centering
    \caption{Mapa Geográfico de Distribuição - Modelo 1 (Sem Intermédio de Estados) sem penalidade}
...
\begin{figure}[H]
    \centering
    \caption{Mapa Geográfico de Distribuição - Modelo 1 (Sem Intermédio de Estados) com penalidade}
    \includegraphics[width=12.0cm,height=12.0cm]{figuras/mapa_m1_true.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:mapa_m1_true}
\end{figure}

A diferença entre os dois mapas evidencia a influência da penalidade na distribuição de vacinas. No mapa sem penalidade (Figura \ref{fig:mapa_m1_false}), observa-se uma rede de distribuição mais descentralizada e dispersa pelo território nacional, já que  temos trocas de vacinas entre municípios de regiões adjacentes entre si, por exemplo, trocas entre municípios da região Norte e do Nordeste. Em contrapartida, no mapa com penalidade (Figura \ref{fig:mapa_m1_true}), destaca-se visualmente uma forte concentração de vetores aéreos (linhas tracejadas) de longa distância e grande espessura partindo da região Sudeste em direção direta às regiões Norte e Nordeste, ou seja, uma conexão entre regiões não adjacentes entre si. Isso consolida a priorização matemática do \textit{solver} em escoar estoques críticos o mais rápido possível mesmo que isso signifique ativar rotas mais longas.

\subsection{Resultados do Modelo 2 (Com Intermédio de Estados)}

\begin{figure}[H]
    \centering
    \caption{Os 10 Maiores Tempos Por Rota - Modelo 2 (Com Intermédio de Estados)}
    \includegraphics[width=16.0cm,height=12.5cm]{figuras/10-maiores_tempos_mod2.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:grafico12_powerbi}
\end{figure}

Conforme ilustrado na \autoref{fig:grafico12_powerbi}, no cenário sem penalidade, os arcos mais demorados concentram-se no abastecimento de estados da Região Norte e parte do Nordeste a partir do Centro-Oeste e de capitais nordestinas (ex: Campo Grande $\to$ AC, Campo Grande $\to$ AM, Goiânia $\to$ AM, São Luís $\to$ RR). O tempo físico dessas rotas satura de forma homogênea na faixa de 5 a 6 horas.
...
\begin{figure}[H]
    \centering
    \caption{Os 10 Menores Tempos Por Rota - Modelo 2 (Com Intermédio de Estados)}
    \includegraphics[width=16.0cm,height=12.5cm]{figuras/12-menores_tempos_mod2.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:grafico15_powerbi}
\end{figure}

Na \autoref{fig:grafico15_powerbi}, a estabilidade frente à penalidade se repete, ou seja, o conjunto das 10 rotas mais curtas é idêntico nos dois casos.

No modelo 2, a diferença estrutural em relação ao Modelo 1 (sem intermédio de estados) está na composição desse ranking. O gráfico mescla rotas intramunicipais ($j \to p$) com fluxos do tipo $k \to j$ cuja presença decorre de os centros de distribuição estaduais poderem estar localizados próximos às capitais do mesmo estado.
...
\begin{figure}[H]
    \centering
    \caption{Informações do Terminal Com Python - Modelo 2 (Com Intermédio de Estados) com penalidade}
    \includegraphics[width=15.0cm,height=5.3cm]{figuras/modelo_2_true.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:terminal10_python}
\end{figure}

Considerando a \autoref{fig:terminal9_python} e a \autoref{fig:terminal10_python}, exceto pela etapa $ik$, verifica-se que rotas gargalo e seus detalhamentos são exatamente iguais entre uma versão de modelo sem penalidade e sua versão com penalidade. O tipo de vacina da entrega mais demorada é a mesma entre as duas saídas. Todas as demandas foram atendidas.
...
\begin{figure}[H]
    \centering
    \caption{Mapa Geográfico de Distribuição - Modelo 2 (Com Intermédio de Estados) sem penalidade}
...
\begin{figure}[H]
    \centering
    \caption{Mapa Geográfico de Distribuição - Modelo 2 (Com Intermédio de Estados) com penalidade}

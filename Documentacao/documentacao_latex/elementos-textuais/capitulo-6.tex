...
Foi gerado um csv com dados globais de cada modelo, o qual foi utilizado para a geração dos gráficos de comparação entre os modelos. Esse csv contém os seguintes dados dispostos nas seguintes colunas:
...
Também foram gerados csvs específicos para cada modelo, os quais foram utilizados para a geração dos gráficos de resultados individuais dos modelos.
...
Os tempos máximos são aproximados pelo valor Limite Superior. 
...
Entre os modelos 1 e 2, ambos com penalidade, a diferença entre os tempos é de 2,7 horas. Isso significa que o modelo 2 tem um tempo $21\%$ maior.
...
Para a obtenção dos dados desse gráfico foi gerada uma média ponderada para os dados de custo tendo a quantidade de doses movimentadas como peso. Para os modelos o valor não variou muito. Isso denota que provavelmente, o solver utilizou um conjunto de rotas parecidos entre os modelos para entregar quantidades parecidas de doses.
...
\begin{figure}[H]
    ...
    \includegraphics[width=15cm,height=9.0cm]{figuras/6-percentual_modais.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
...
Verifica-se predomínio, em, aproximadamente 70\%, das rotas do tipo jp, na qual o município realiza o abastecimento de seus postos. O restante (aproximadamente 25\%) é preenchido pelas rotas do tipo ij, o que corresponde a casos em que um município precisou pegar doses da oferta de outro município.

Vacina do maior tempo de entrega e rotas gargalo por etapa:

\begin{figure}[H]
    \centering
    \caption{Informações do Terminal Com Python - Modelo 1 s/ penalidade}
    \includegraphics[width=8.3cm,height=3.0cm]{figuras/modelo_1_false.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
...
\begin{figure}[H]
    \centering
    \caption{Informações do Terminal Com Python - Modelo 1 c/ penalidade}
    \includegraphics[width=8.3cm,height=3.0cm]{figuras/modelo_1_true.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
...
\begin{figure}[H]
    \centering
    \caption{Mapa Geográfico de Distribuição - Modelo 1 s/ penalidade}
...
\begin{figure}[H]
    \centering
    \caption{Mapa Geográfico de Distribuição - Modelo 1 c/ penalidade}
...
\begin{figure}[H]
    ...
    \includegraphics[width=16cm,height=5.0cm]{figuras/8-percentual_rotas_mod2.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:grafico13_powerbi}
\end{figure}

Verifica-se predomínio, em pouco mais da metade, do uso das rotas do tipo jp, na qual o município realiza o abastecimento de seus postos. O tipo de ik aparece em segundo lugar com aproximadamente 20\% e esse número denota que, na maioria das vezes não foi necessário usar esse tipo de rota tipicamente acionada quando é necessário usar a oferta de outro município. O restante (aproximadamente 15\%) é preenchido pelas rotas do tipo kj, na qual o Estado realiza o abastecimento de sua capital. 

Vacina do maior tempo de entrega e rotas gargalo por etapa:

\begin{figure}[H]
    \centering
    \caption{Informações do Terminal Com Python - Modelo 2 s/ penalidade}
    \includegraphics[width=9.3cm,height=3.5cm]{figuras/modelo_2_false.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
...
\begin{figure}[H]
    \centering
    \caption{Informações do Terminal Com Python - Modelo 2 c/ penalidade}
    \includegraphics[width=9.3cm,height=3.5cm]{figuras/modelo_2_true.png} 
    \legend{Fonte: Elaborado pelo autor (2026)}
    \label{fig:terminal10_python}
\end{figure}

Excento pela etapa ik, verifica-se que rotas gargalo e seus detalhamentos são exatamente iguais entre uma versão de modelo sem penalidade e sua versão com penalidade. O tipo de vacina da entrega mais demorada é a mesma entre as duas saídas. Todas as demandas foram atendidas.
...
\begin{itemize}
    ...
    \item Cenário com penalidade: \url{https://mapa-rotas-m2-penalidade.netlify.app}
\end{itemize}

\begin{figure}[H]
    \centering
    \caption{Mapa Geográfico de Distribuição - Modelo 2 s/ penalidade}
